...
[{icc_PDFA} currentfile /ASCII85Decode filter /PUT pdfmark
...
`;
  ...
  const { generateFacturXXml } = await import("../../api/src/invoices/facturx.generator");
  ...
  // Mark the attachment as /AFRelationship /Alternative
  const namesDict = doc.catalog.lookup(PDFName.of("Names")) as any;
  if (namesDict?.lookup) {
    const embeddedFiles = namesDict.lookup(PDFName.of("EmbeddedFiles"));
    const efNames = embeddedFiles?.lookup?.(PDFName.of("Names"));
    const entries = efNames?.asArray?.() ?? [];
    for (let i = 1; i < entries.length; i += 2) {
      const fileSpec = entries[i]?.lookup?.() ?? entries[i];
